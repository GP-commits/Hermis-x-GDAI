...
import { COLLECTION, BOARD_LIMIT, compareScores, displayName, validScore } from './leaderboard-model.js';
...
const outboxKey = uid => `duskride:score-outbox:${uid}`;
...
export class LeaderboardService extends EventTarget {
  constructor() {
    ...
    this.guestBest = read('duskride:guest-score');
  ...
  snapshot() { return { configured: this.configured, ready: this.ready, user: this.user, rows: this.rows, best: this.best, pending: this.pending, guestBest: this.guestBest, connection: this.connection, sync: this.sync, error: this.error }; }
        ...
        await new Promise(resolve => {
          ...
          authSDK.onAuthStateChanged(this.auth, account => {
            ...
            if (account && this.claimGuest && this.guestBest) {
              ...
              this.guestBest = null; write('duskride:guest-score', null); this.claimGuest = false; this.persist();
            }
            ...
            if (this.pending && account) this.schedule(0);
  ...
  async watch() { this.wantWatch = true; if (await this.init()) { if (this.wantWatch) this.startListeners(); } else this.emit(); }
  stopWatching() { this.wantWatch = false; this.unsubscribeRows?.(); this.unsubscribeBest?.(); this.unsubscribeRows = this.unsubscribeBest = null; }
  startListeners() {
  ...
  record(candidate, immediate = false) {
    ...
    if (!this.user) {
      if (compareScores(candidate, this.guestBest) > 0) {
        ...
        if (changedScore || immediate || Date.now() - this.lastPersist > 5000) { write('duskride:guest-score', candidate); this.lastPersist = Date.now(); }
      }
  ...
  persist() {
    ...
    if (!this.user && this.guestBest) write('duskride:guest-score', this.guestBest);
  ...
  async flush() {
    if (!this.ready || !this.user || !this.pending || this.inFlight) return;
    if (!navigator.onLine) { this.sync = 'offline'; this.persist(); this.emit(); return; }
    const user = this.user, candidate = { ...this.pending };
